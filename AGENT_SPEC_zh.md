...
```text
agents/
└── <AgentName>/                 # Agent 根目录 (使用 PascalCase，如 kuko)
    ├── README.md                # [必需] 统一文档
    ├── opencode/                # [必需] OpenCode 平台专用配置
    │   ├── agent.md             # Agent 定义 (原 <agent_name>.md)
    │   └── config.json          # OpenCode 配置 (原 opencode.json)
    ├── claude/                  # [可选] Claude Code 平台专用配置
    │   └── agent.md             # Claude Agent 定义 (原 CLAUDE.md)
    ├── shared/                  # [必需] 共享资源
    │   ├── SKILLS/              # 技能模块目录
    │   │   └── <skill-name>/
    │   │       └── SKILL.md
...
2.  **复制模板**: 从 `agents/kuko` 复制 `opencode.json` 和结构作为起点。
